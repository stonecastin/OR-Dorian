...
twitter_vars <- list(
  app = "DorianGEOG323sem7",
  key = "3159UZAWIWJ7s5sRuN2NDZEQt",
  secret = "0NYi9mPW1SeTVJCrHdmp2eC8XeLo3AoJr3WY6Q7oz6RYpBPNUj"
)
...
dorian_text <- dorian %>%
  select(text) %>%
  plain_tweets() #removes non language, part of rtweet package
...
data("stop_words") #stop words is from tidytext
...
census_api_key(dlgInput(
  "fd77230520f2fa9bb485f1843f616487875bc4f9",
...
```{r}
counties <- counties %>%
  mutate(dorrate_log = log(dorrate))
```

# our new map: normalized difference
```{r}
#people tweeting about hurricane, dorian, sharpiegate more than in nov.
ggplot() +
  geom_sf(data = counties, 
    aes(fill = cut_interval(dorrate_log, 5)), color = "grey", lwd=0.1) +
  scale_fill_brewer(palette = "GnBu") +
  guides(fill = guide_legend(title = "Activity Increase")) +
  labs(title = "Level of Incresed activity of hurricane, dorian, sharpiegate") +
  theme(
    plot.title = element_text(hjust = 0.5),
    axis.title.x = element_blank(),
    axis.title.y = element_blank()
